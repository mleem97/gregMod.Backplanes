...
using GregMod.Backplanes;

namespace GregMod.Backplanes;

// Standalone UI layer (no gregCore): own UIDocument, toasts,
// input guard, font. Only used when GregHost.HasCore == false.
// All methods free of gregCore types (JIT-safe).
public static class ModLocalUI
{
    ...
    public static VisualElement GetLayerRoot()
    {
        ...
        try
        {
            var go = new GameObject("gregBackplanesUILayer");
        ...
        {
            MelonLogger.Warning("[Backplanes] Layer build failed: " + ex.Message);
            return null;
        }
    }
